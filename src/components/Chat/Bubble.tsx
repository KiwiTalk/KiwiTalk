...
const Bubble: React.FC<{hasTail: boolean, author?: string, time?: string}> = ({hasTail, author, time, children}) => {
  return (
    <Wrapper>
      {(() => hasTail ? <BubbleTail src={bubbleTail}/> : <FakeTail />)()}
      <Content>
        {(() => author != null ? <Author>{author}</Author> : undefined)()}
        {children}
      </Content>
      {(() => time != null ? <Date>{time}</Date> : undefined)()}
    </Wrapper>
  );
};
